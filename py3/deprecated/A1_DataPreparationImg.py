...
trn_plots = ["B1_6_0003", "B1_6_0023", "B1_6_0063", "B1_6_0078", "B1_6_0086",\
             ...
             "B2_2_0053"]
tst_plots = ["B1_6_0286", "B1_6_0267", "B2_2_0061"]
...
for set_X, set_y, use_plots in zip(["X", "X_tst"], ["y", "y_tst"],\
    ...
    for plot in use_plots:
        ...
        downloadShapefiles(plot_id = plot,
                               ...
                               dateTime = last_modified)
    ## get class IDs
    classes_decoded = get_classes(dir_shp(myear = year), plots = all_plots)# use all plots so the class number is not overwritten by test set
    ...
    for plot in use_plots:
        ...
        if check_version(file = list(pathlib.Path(dir_tls(plot_id = plot,
            ...
            ### delete old files
            old_X_tiles = list(pathlib.Path(dir_tls(myear = year, dset = set_X)) \
                               .glob("**/" + plot + "*"))
            old_y_tiles = list(pathlib.Path(dir_tls(myear = year, dset = set_y)) \
                               .glob("**/" + plot + "*"))
